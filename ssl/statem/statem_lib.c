...
CON_FUNC_RETURN tls_construct_cert_verify(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    printf("[DUAL_SIGN_SERVER] Classic key size: %d bytes\n", EVP_PKEY_size(pkey));
    ...
    if (s->cert->dual_certs_enabled && s->cert->pqkey != NULL) {
        ...
        printf("[DUAL_SIGN_SERVER] PQC key type: %d\n", EVP_PKEY_get_id(pq_pkey));
        printf("[DUAL_SIGN_SERVER] PQC key size: %d bytes\n", EVP_PKEY_size(pq_pkey));
        ...
        if (pq_lu->sig == EVP_PKEY_RSA_PSS && EVP_PKEY_get_id(pq_pkey) == EVP_PKEY_RSA) {
            if (EVP_PKEY_CTX_set_rsa_padding(pq_pctx, RSA_PKCS1_PSS_PADDING) <= 0
                || EVP_PKEY_CTX_set_rsa_pss_saltlen(pq_pctx,
                                                    RSA_PSS_SALTLEN_DIGEST) <= 0) {
                printf("[DUAL_SIGN_SERVER] ERROR: Failed to set PQC RSA PSS parameters\n");
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_EVP_LIB);
                goto err;
            }
        } else {
...
MSG_PROCESS_RETURN tls_process_cert_verify(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    if (s->session->peer_pqc_chain != NULL && sk_X509_num(s->session->peer_pqc_chain) > 0) {
        ...
        if (pq_pkey != NULL) {
            ...
            printf("[DUAL_SIGN_CLIENT] PQC key size: %d bytes\n", EVP_PKEY_size(pq_pkey));
            pq_mctx = EVP_MD_CTX_new();
            if (pq_mctx == NULL) {
    ...
    if (pq_pkey != NULL) {
        /* For PQC, we'll use the same signature algorithm for now */
        if (!tls1_lookup_md(sctx, s->s3.tmp.peer_sigalg, &pq_md)) {
