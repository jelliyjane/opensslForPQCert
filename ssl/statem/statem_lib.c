...
int ssl3_do_write(SSL_CONNECTION *s, uint8_t type) {
  ...
  /* Catalyst logging: Track message writing */
  OSSL_TIME now = ossl_time_now();
  uint64_t sec = ossl_time2seconds(now);
  uint64_t usec = ossl_time2us(now) % 1000000;
  fprintf(stderr,
          "[CATALYST] %llu.%06llu ssl3_do_write: type=%d, size=%zu, "
          "hand_state=%d\n",
          (unsigned long long)sec, (unsigned long long)usec, type, s->init_num,
          s->statem.hand_state);
  ...
  if (written == s->init_num) {
    ...
    /* Catalyst logging: Message write completed */
    now = ossl_time_now();
    sec = ossl_time2seconds(now);
    usec = ossl_time2us(now) % 1000000;
    fprintf(stderr,
            "[CATALYST] %llu.%06llu ssl3_do_write: COMPLETED, written=%zu\n",
            (unsigned long long)sec, (unsigned long long)usec, written);
...
static int get_cert_verify_tbs_data(SSL_CONNECTION *s, unsigned char *tls13tbs,
  ...
  if (SSL_CONNECTION_IS_TLS13(s)) {
    ...
    if (s->statem.hand_state == TLS_ST_CR_CERT_VRFY ||
        ...
        s->statem.hand_state == TLS_ST_SW_PQCERT_VRFY ||
        s->statem.hand_state == TLS_ST_CR_PQCERT_VRFY)
...
CON_FUNC_RETURN tls_construct_cert_verify(SSL_CONNECTION *s, WPACKET *pkt) {
  ...
  fprintf(stderr, "[%lu.%06lu] tls_construct_cert_verify end\n",
          (unsigned long)(end_us / 1000000), (unsigned long)(end_us % 1000000));
...
int ssl_choose_client_version(SSL_CONNECTION *s, int version,
  ...
  } else if (!SSL_CONNECTION_IS_DTLS(s) && s->version < TLS1_2_VERSION &&
             real_max > s->version) {
