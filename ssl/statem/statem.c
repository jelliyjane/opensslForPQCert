...
static int state_machine(SSL_CONNECTION *s, int server) {
  ...
  if (st->state == MSG_FLOW_UNINITED || st->state == MSG_FLOW_FINISHED) {
    ...
    fprintf(stderr,
            ...
            "hand_state=%d\n",
            (unsigned long long)sec, (unsigned long long)usec, server,
            st->hand_state);
  ...
  while (st->state != MSG_FLOW_FINISHED) {
    if (st->state == MSG_FLOW_READING) {
      ...
      if (ssret == SUB_STATE_FINISHED) {
        /* Catalyst logging: State transition */
        now = ossl_time_now();
        sec = ossl_time2seconds(now);
        usec = ossl_time2us(now) % 1000000;
        fprintf(stderr,
                "[CATALYST] %llu.%06llu state_machine: READING->WRITING, "
                "hand_state=%d\n",
      ...
      if (ssret == SUB_STATE_FINISHED) {
        ...
        usec = ossl_time2us(now) % 1000000;
        fprintf(stderr,
                "[CATALYST] %llu.%06llu state_machine: WRITING->READING, "
                "hand_state=%d\n",
                (unsigned long long)sec, (unsigned long long)usec,
                st->hand_state);
        ...
        /* Catalyst logging: Handshake end */
        now = ossl_time_now();
        sec = ossl_time2seconds(now);
        usec = ossl_time2us(now) % 1000000;
        fprintf(stderr,
                "[CATALYST] %llu.%06llu state_machine: HANDSHAKE_END, "
                "hand_state=%d\n",
                (unsigned long long)sec, (unsigned long long)usec,
                st->hand_state);
