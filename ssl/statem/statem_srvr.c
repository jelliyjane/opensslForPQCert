...
CON_FUNC_RETURN tls_construct_server_certificate(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    case TLSEXT_cert_type_x509:
        ...
        if (s->cert->dual_certs_enabled && s->cert->pqkey != NULL) {
            ...
            /* Output classic certificate chain */
            ...
            printf("[DUAL_CERT_SERVER] Delimiter added successfully\n");
            
            printf("[DUAL_CERT_SERVER] Encoding PQC certificate chain\n");
            /* Output PQC certificate chain */
            if (!ssl3_output_cert_chain(s, pkt, s->cert->pqkey, 0)) {
            ...
            if (!WPACKET_close(pkt)) {
                printf("[DUAL_CERT_SERVER] ERROR: Failed to close certificate list\n");
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
                return 0;
            }
